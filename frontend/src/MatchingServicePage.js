...
function MatchingServicePage() {
    ...
    useEffect(() => {
        ...
        function onFoundMatch(data) {
            console.log("Matching service found match, room allocated:");
            console.log(data);
        }
        ...
        function onWaitingInRoom() {
            console.log("Waiting in room");
        }

        function onAllJoinedRoom(data) {
        ...
        socket.on('waiting-in-room', onWaitingInRoom);
        socket.on('all-joined-room', onAllJoinedRoom);
        ...
        return () => {
            ...
            socket.off('waiting-in-room', onWaitingInRoom);
            socket.off('all-joined-room', onAllJoinedRoom);
        };
    ...
    const joinRoom = () => {
        ...
        socket.emit('join-room', selectedUser.email);
    }
