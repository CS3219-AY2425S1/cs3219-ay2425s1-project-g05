...
//     it("Respond with 200 status code with correct ids with no overlap id", async function () {
//         await Question.create(sampleQuestions.validQuestion1);
//         await Question.create(sampleQuestions.validQuestion3);

//         const res = await request(app)
//             .get(`/api/question-service/categories`)
//             .expect(200)
//             .expect('Content-Type', /json/);
//             console.log(res.body.data.categories.categories);
//         assert.deepEqual(res.body.data.categories.categoriesId, [0, 1, 3, 4], 'Wrong categoriesId mapped');
//         assert.deepEqual(res.body.data.categories.categories, ['ARRAYS', 'ALGORITHMS', "DATA STRUCTURES", "BRAINTEASER"], 'Wrong categories mapped');

//         assert.equal(res.body.message, "Categories obtained successfully", 'Response length does not match');
//     });


//     it("Respond with 200 status code with correct ids with overlap id", async function () {
//         await Question.create(sampleQuestions.validQuestion1);
